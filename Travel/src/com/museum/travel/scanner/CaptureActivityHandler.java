...
public final class CaptureActivityHandler extends Handler {
	...
	public void handleMessage(Message message) {
		switch (message.what) {
		case CaptureActivity.MSG_RESTART_PREVIEW:
			Log.d(TAG, "Got restart preview message");
			restartPreviewAndDecode();
			break;
		case CaptureActivity.MSG_DECODE_SUCCEEDED:
			...
			break;
		case CaptureActivity.MSG_DECODE_FAILED:
			// We're decoding as fast as possible, so when one decode fails,
			// start another.
			state = State.PREVIEW;
			cameraManager.requestPreviewFrame(decodeThread.getHandler(),
					CaptureActivity.MSG_DECODE);
			break;
		case CaptureActivity.MSG_RETURN_SCAN_RESULT:
			Log.d(TAG, "Got return scan result message");
			activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
			activity.finish();
			activity.overridePendingTransition(0, 0);
			break;
		case CaptureActivity.MSG_LAUNCH_PRODUCT_QUERY:
	...
	public void quitSynchronously() {
		...
		Message quit = Message.obtain(decodeThread.getHandler(), CaptureActivity.MSG_QUIT);
		quit.sendToTarget();
		try {
		...
		removeMessages(CaptureActivity.MSG_DECODE_SUCCEEDED);
		removeMessages(CaptureActivity.MSG_DECODE_FAILED);
	}
